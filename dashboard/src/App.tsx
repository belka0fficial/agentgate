...
const nav = [
  ...
  { to: '/chats', label: 'Chats', icon: MessageSquare, hint: 'Sessions with Hermes' },
...
const routeMeta = [
  { match: '/settings/character', label: 'Character', kicker: 'Identity', note: 'Hermes voice, posture, and boundaries.' },
  ...
  { match: '/chats', label: 'Chats', kicker: 'Conversation', note: 'Hermes sessions, forks, and active context.' },
...
function Sidebar({ mobile, close, summary, theme, toggleTheme }: { mobile?: boolean, close?: () => void, summary?: Any, theme: Theme, toggleTheme: () => void }) {
  ...
  const systemsOnline = ['hermes', 'toolgate', 'memorygate'].filter(name => !summary?.health?.[name]?.error).length
          ...
          <Card className="chat-rail">
            ...
            <div className="chat-rail-stack">
              ...
              <div className="chat-rail-card">
                ...
                <small>{streaming ? 'Hermes is generating now' : 'Ready for the next prompt'}</small>
              </div>
              <div className="chat-rail-card">
